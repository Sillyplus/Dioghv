...
extension Keyboard {
    ...
    class func dKeyboard() -> Keyboard {
        let kb = Keyboard()
        
        for key in ["Q", "W", "E", "R", "T", "Y", "U", "I", "O", "P"] {
            let keyModel = Key(.character)
            keyModel.setLetter(key)
            kb.addKey(keyModel, row: 0, page: 0)
        }
        
        for key in ["A", "S", "D", "F", "G", "H", "J", "K", "L"] {
            let keyModel = Key(.character)
            keyModel.setLetter(key)
            kb.addKey(keyModel, row: 1, page: 0)
        }
        
        let keyModel = Key(.shift)
        kb.addKey(keyModel, row: 2, page: 0)
        ...
        let backspace = Key(.backspace)
        kb.addKey(backspace, row: 2, page: 0)
        
        let keyModeChangeNumbers = Key(.modeChange)
        keyModeChangeNumbers.uppercaseKeyCap = "123"
        keyModeChangeNumbers.toMode = 1
        kb.addKey(keyModeChangeNumbers, row: 3, page: 0)
        
        let keyboardChange = Key(.keyboardChange)
        kb.addKey(keyboardChange, row: 3, page: 0)

        let space = Key(.space)
        space.uppercaseKeyCap = "space"
        space.uppercaseOutput = " "
        space.lowercaseOutput = " "
        kb.addKey(space, row: 3, page: 0)
        
        let returnKey = Key(.return)
        returnKey.uppercaseKeyCap = "return"
        returnKey.uppercaseOutput = "\n"
        returnKey.lowercaseOutput = "\n"
        kb.addKey(returnKey, row: 3, page: 0)
        
        for key in ["1", "2", "3", "4", "5", "6", "7", "8", "9", "0"] {
            let keyModel = Key(.specialCharacter)
            keyModel.setLetter(key)
            kb.addKey(keyModel, row: 0, page: 1)
        }
        
        for (key, letter) in [("-", "-"), ("/", "／"), (":", "："), (";", "；"), ("(", "（"), (")", "）"), ("¥", "¥"), ("&", "&"), ("@", "@"), ("\"", "“")] {
            let keyModel = Key(.specialCharacter)
            keyModel.setLetter(key)
            keyModel.lowercaseOutput = letter
            keyModel.uppercaseOutput = letter
            kb.addKey(keyModel, row: 1, page: 1)
        }
        
        let keyModeChangeSpecialCharacters = Key(.modeChange)
        keyModeChangeSpecialCharacters.uppercaseKeyCap = "#+="
        keyModeChangeSpecialCharacters.toMode = 2
        kb.addKey(keyModeChangeSpecialCharacters, row: 2, page: 1)
        ...
        for (key, letter) in [("。", "。"), (",", "，"), ("?", "？"), ("!", "！"), ("'", "‘")] {
            ...
            keyModel.lowercaseOutput = letter
            keyModel.uppercaseOutput = letter
            kb.addKey(keyModel, row: 2, page: 1)
        }
        
        kb.addKey(Key(backspace), row: 2, page: 1)
        
        let keyModeChangeLetters = Key(.modeChange)
        keyModeChangeLetters.uppercaseKeyCap = "ABC"
        keyModeChangeLetters.toMode = 0
        
        kb.addKey(keyModeChangeLetters, row: 3, page: 1)
        kb.addKey(Key(keyboardChange), row: 3, page: 1)
        kb.addKey(Key(space), row: 3, page: 1)
        kb.addKey(Key(returnKey), row: 3, page: 1)
        
        for key in ["[", "]", "{", "}", "#", "%", "^", "*", "+", "="] {
            let keyModel = Key(.specialCharacter)
            keyModel.setLetter(key)
            kb.addKey(keyModel, row: 0, page: 2)
        }
        
        for key in ["_", "\\", "|", "~", "<", ">", "€", "£", "$", "•"] {
            let keyModel = Key(.specialCharacter)
            keyModel.setLetter(key)
            kb.addKey(keyModel, row: 1, page: 2)
        }
        
        kb.addKey(Key(keyModeChangeNumbers), row: 2, page: 2)
        ...
        for (key, letter) in [("。", "。"), (",", "，"), ("?", "？"), ("!", "！"), ("'", "‘")] {
            ...
            keyModel.lowercaseOutput = letter
            keyModel.uppercaseOutput = letter
            kb.addKey(keyModel, row: 2, page: 2)
        }
        
        kb.addKey(Key(backspace), row: 2, page: 2)
        
        kb.addKey(Key(keyModeChangeLetters), row: 3, page: 2)
        
        kb.addKey(Key(keyboardChange), row: 3, page: 2)
        
        kb.addKey(Key(space), row: 3, page: 2)
        
        kb.addKey(Key(returnKey), row: 3, page: 2)
